...
4. Read the ADRs under `doc/adr/` that govern the area you intend to change.  The
   generated `doc/adr/README.md` index may be absent in a fresh checkout.
...
- `doc/adr/README.md`: ignored ADR landing page generated by `make adr-index` or
  `make docs`; never commit it as maintained source.
...
- `make adr-index` is offline and generates linked ADR navigation from prepared
  adrctl state.
- `make docs` is offline and consumes the prepared Doxygen filter and adrctl.
...
release-artifact and checksum-companion contract.  ADR-017 governs generated ADR
navigation and keeps adrctl outside runtime and release artifacts.
...
The ADR landing page follows a separate maintained/generated ownership model.
`README.intro.md` and `README.outro.md` are maintained source; the linked list and
assembled `doc/adr/README.md` are generated from the ADR corpus by the pinned
adrctl dependency.  Routine documentation generation produces textual ADR
navigation only and does not generate a relationship graph.

See ADR-007, ADR-008, ADR-010, ADR-017, and `doc/documentation-standard.md`.
...
Documentation validation should prove that `make docs` does not acquire missing
dependencies, that the generated ADR landing page and Doxygen output are ignored
state, and that documentation generation leaves tracked repository state clean.
