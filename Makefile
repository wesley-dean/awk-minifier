...
# Build remains offline, but steady-state minification requires the pinned
# previous release to have been prepared through make deps.
build: verify-build-deps
	$(MAKE) --no-print-directory $(DIST_SCRIPTS) $(DIST_CHECKSUMS)

verify-build-deps:
	@test -r "$(AWK_MINIFIER)" || { \
		printf '%s\n' 'Missing vendor/awk-minifier.awk; run make deps or make all before make build' >&2; \
		exit 1; \
	}
...
$(DIST_MIN_SCRIPT): $(DIST_SCRIPT) $(AWK_MINIFIER)
	...
	{ \
		...
		printf '%s\n' '# Minifier input: awk-minifier.awk body'; \
		printf '%s\n' '# End generated header.'; \
		cat "$$body"; \
	} >"$@.tmp"; \
	chmod 0755 "$@.tmp"; \
